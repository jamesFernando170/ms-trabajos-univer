...
export class DirectorStrategy implements AuthenticationStrategy {
  ...
  async authenticate(request: Request): Promise<UserProfile | undefined> {
    ...
    if (token) {
      let url = `${Keys.url_validar_token}?${Keys.arg_token}=${token}&${Keys.arg_rol_validar}=${Keys.rol_director}`;
      console.log(url)
      let respuesta = "";
      await fetch(url)
        .then(async (res: any) => {
